...
class NodePDFToMD(NodeBase):
    ...
    name = "node_pdf_to_md"
    def local_file_upload(self, state: ImportGraphState):
        # 防御
        ...
        logger.info("向服务器发送请求成功")
        # 反序列化取值
        result = response.json()
        # print(result)
        if result["code"] != 0:
            logger.error("向服务器发送请求,请求数据返回失败")
        logger.info("向服务器发送请求,请求数据返回成功")
        ...
        return batch_id,pdf_path_obj,local_dir_obj


    def get_zip_urls(self, state: ImportGraphState,batch_id):
        # 1.post提交,get查询,这里要进行轮询,一直请求直到服务器给结果
        import time
        import requests
        token = MinerUConfig.MINERU_API_KEY
        batch_id = batch_id
        ...
        while True:
            ...
            try:
                ...
                if extract_result['state'] != "done":
                    logger.info(f"Mineru正在解析您的文件中...请稍后")
                    raise Exception("Mineru正在解析您的文件中...请稍后")
    ...
    def download_and_unzip(self, state: ImportGraphState, md_zip_urls, pdf_path_obj, local_dir_obj):
        ...
        logger.info(f"{pdf_path_obj.stem}解压成功")
    ...
    def process(self, state: ImportGraphState):
        ...
        """
        #1.本地文件批量上传
        batch_id,pdf_path_obj,local_dir_obj = self.local_file_upload(state)

        #2.结果解析获取解压资源地址
        md_zip_urls = self.get_zip_urls(state,batch_id)

        #3.下载解压重命名保存一条龙
        new_unzip_path_obj=self.download_and_unzip(state, md_zip_urls, pdf_path_obj, local_dir_obj)
...
if __name__ == '__main__':
    pdf_list = [
        r"E:\AI大模型\第七阶段 掌柜智库\资料\05-设备手册汇总\doc\hak180产品安全手册.pdf",
        r"E:\AI大模型\第七阶段 掌柜智库\资料\05-设备手册汇总\doc\万用表RS-12的使用.pdf",
        r"E:\戴炜\衢丽\图纸及传真\（盖章扫描）中铁十一局衢丽铁路小南海制梁CARS-QL-2-XNH-003号.pdf",
        r"C:\Users\Administrator\Desktop\调档函.pdf"
    ]
