...
class NodeRerank(NodeBase):
    ...
    def process(self, state: QueryGraphState):
        ...
        if not rrf_chunks or not web_search_docs:
            logger.error("Rerank节点接收数据失败")
            raise Exception("Rerank节点接收数据失败")
        ...
        if not rewritten_query:
            logger.info("接收rewritten_query失败")
            raise Exception("接收rewritten_query失败")

        texts =[merge_chunk.get("content") for merge_chunk in merge_chunks]
        res = rerank(query = rewritten_query,texts = texts,limit = 10)
        # print(res)
        for item in res:
