...
class NodeRrf(NodeBase):
    ...
    def process(self, state: QueryGraphState):
        ...
        if not embedding_chunks or not hyde_embedding_chunks:
            logger.error("RRF节点接收数据失败")
            raise ValueError("RRF节点接收数据失败")

        weight_embedding = [
            (embedding_chunks,1),(hyde_embedding_chunks,1)
        ]
        ...
        for chunks , weight in weight_embedding:
            for idx, chunk in enumerate(chunks,start =1):
                ...
                if chunk_id in rrf_chunks:
                    rrf_chunks["id"]["score"] += chunk_score
                else:
                    chunk["score"] = chunk_score
                    rrf_chunks.append({"id":chunk})
        rrf_chunks = sorted(rrf_chunks,key = lambda x : x["id"]["score"],reverse = True)

        return {
            "rrf_chunks": [rrf_chunk.get("id") for rrf_chunk in rrf_chunks]
        }
